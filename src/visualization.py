...
def plot_mosaic_overview(df, group_col='risk_group', target_col='stroke'):
    ...
    Updated: Percentages INSIDE the boxes, Black borders.
    """
    ...
    try:
        ...
        # 1. Prepare Label Map (create a contingency table of Group vs Stroke Status, normalize by row (index) to get proportions, and convert to percentages.)
        cross_props = pd.crosstab(df[group_col], df[target_col], normalize='index') * 100
        label_map = {}
        # Populate a dictionary mapping (Group, Stroke Status) keys 
        for group in cross_props.index:
            # Convert keys to (str, str) format to match mosaic's internal keys if column exists
        ...
        # 2. Define Properties (Colors & Borders)
        def props(key):
            # Check if the tile represents a Stroke case ('1') to assign Red color
            is_stroke = str(key[1]) == '1'
            return {
                'color': '#d62828' if is_stroke else '#8ecae6', 
                'alpha': 0.9, 
                'edgecolor': 'black', # Adds a black border around each tile
                'linewidth': 1.0      
            }

        # 3. Define Labelizer (The text inside)
        def labelizer(key):
            # Retrieve the pre-calculated percentage string for the current mosaic tile
            # key comes as (str, str) from mosaic
            k = (str(key[0]), str(key[1]))
            return label_map.get(k, "") # Safely retrieve the label from the map, returning an empty string if the key is missing.

        # Generate Plot
        mosaic(df, [group_col, target_col], properties=props, gap=0.02, 
               title='Mosaic Plot: Sample Size vs Outcome', labelizer=labelizer)

        plt.xlabel('Risk Group (Width = Sample Size)')
        plt.ylabel('Outcome (Height = Proportion)')
        
        # Create a custom legend handle (colored patch) to represent 'Healthy' subjects
        ...
        plt.legend(handles=legend_elements, loc='upper right', title="Outcome")
